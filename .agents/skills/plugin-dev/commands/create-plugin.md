...
Guide the user through creating a complete, high-quality plugin from initial concept to tested implementation. Follow a systematic approach: understand requirements, design components, clarify details, implement following best practices, validate, and test.
...
- **Use tasks**: Track all progress throughout all phases
...
**Actions**:
1. Create task list with all 7 phases
...
**Actions**:
1. Analyze plugin requirements and determine needed components:
...
2. For each component type needed, identify:
   - How many of each type
   - What each one does
   - Rough triggering/usage patterns
3. Present component plan to user as table:
   ```
...
2. Choose plugin location:
   ...
   - Offer options: current directory, custom path
...
1. For each command:
   ...
   - Write instructions FOR the agent (not TO user)
   - Provide usage examples and tips
   - Reference relevant skills if applicable

### For Agents:
1. For each agent, use agent-creator pattern:
   - Provide description of what agent should do
   - Generate: identifier, whenToUse with examples, systemPrompt
...
2. Create example settings file (as documentation)
3. Implement settings reading in hooks/commands as needed
4. Add to .gitignore: `*.local.md`
...
3. **Review skills** (if plugin has skills):
   - For each skill, check quality
...
**Goal**: Test that plugin works correctly
...
3. **Testing recommendations**:
   ...
   - For commands: Run commands with various arguments
   - For agents: Create scenarios matching agent examples
   - For hooks: Check hook execution
   - For MCP: Verify servers and tools
...
- **Use tasks** to track progress at every phase
- **Load skills** when working on specific component types
...
- **Apply best practices**:
  ...
  - Commands written FOR the agent
